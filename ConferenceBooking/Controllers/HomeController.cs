        ...
        public ActionResult Index()
{
    ...
    return View(meetings);
        ...
        private List<Meeting> GetMeetingsFromDatabase()
        {
            ...
            using (SqlConnection connection = new SqlConnection(_connectionString))
            {
                string query = "SELECT meeting_id, meeting_title, start_time, end_time, room_id FROM Meetings";
                SqlCommand command = new SqlCommand(query, connection);
        ...
        public ActionResult DeleteMeeting(int id)
        {
            ...
            if (!isAuthorized)
            {
                return Json(new { success = false, message = "You are not authorized to delete this meeting." });
            }
            ...
            return new List<SelectListItem>
    {
        ...
        [HttpPost]
        public ActionResult adminpage(Regis model)
        {
            if (ModelState.IsValid)
            {
                // Check if the email already exists
            ...
            return new List<SelectListItem>
    {
        ...
        public ActionResult AddRoom(Room room)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    // Handle database error
                    return Json(new { success = false, message = "Room addition failed. Please try again. Error: " + ex.Message });
                }
            }
